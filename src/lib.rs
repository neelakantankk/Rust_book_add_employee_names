pub mod employees {
        ...
        for (index, &word) in words.iter().enumerate() {
            if word == "to" {
                if index < words.len() - 1 {
                    let employees_in_dep =
                        employees.entry(words[index + 1..].join(" ")).or_default();

                    employees_in_dep.push(words[1..index].join(" "));
    ...
    fn list_in_department(employees: &HashMap<String, Vec<String>>, department_to_list: String) {
